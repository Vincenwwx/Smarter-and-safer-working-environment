(define (domain office)
    ...
    (:action switchoff_buzzer1
        ...
        :precondition (and (off4 ?b1) (<(movement ?p) (movement_value)))
    ...
    (:action switchon_buzzer1
        :parameters (?b1 - buzzer1 ?p - person ?m - message)
        :precondition (or (not(off4 ?b1)) (>=(movement ?p) (movement_value)))
    ...
    ; person detected & body temp in allowable limit 'greenled on'
    ; and buzzer must say 'Please come in'
    ...
    ; person detected & body temp not in allowable limit 'redled on'
    ; and buzzer mut say 'Sorry, abnormal temperature'
